...
本プロジェクトでは、単にCRUD機能を実装するだけではなく、

「実際のECサイト運用で発生する業務要件をどのようにコードへ落とし込むか」
...
複数の条件を組み合わせてシステムの振る舞いを制御することで、

実運用を意識した業務ロジックを実現しています。

---

## ② ドメインロジックをModelへ集約

業務ロジックをControllerに記述すると、
...
Laravel向け管理画面ライブラリである Filament を採用しました。
...
データ取得と業務ロジックを分離することで、

データベース変更時の影響範囲を最小化できる構成としています。
...
実運用を想定した業務ロジック設計と保守性を重視して開発した点が本プロジェクトの特徴です。
